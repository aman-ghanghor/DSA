...
struct Node* searchRecur(struct Node* root, int value) {
    if(root != NULL) {
        if(root->data == value) {
            return root ;
        }
        else if(value < root->data) {
            searchRecur(root->left, value);
        }
        else if(value > root->data) {
            searchRecur(root->right, value);
        }
        else {
            return NULL;
        }
...
int main()
{
    ...
    */
    ...
    // SEARCHING
    int ele = 50;
    struct Node* searchedElement = searchRecur(p, ele);
